...
def like_change(request):
    ...
    if request.GET.get('is_like') == 'true':
        # 要点赞
        like_record, created = LikesRecord.objects.get_or_create(content_type=content_type, object_id=object_id, user=user)
        if created:
            # 未点赞过，进行点赞
        ...
        if LikesRecord.objects.filter(content_type=content_type, object_id=object_id, user=user).exists():
            # 有点赞过，取消点赞
            like_record = LikesRecord.objects.get(content_type=content_type, object_id=object_id, user=user)
            like_record.delete()
            # 点赞总数减1
            like_count, created = LikesCount.objects.get_or_create(content_type=content_type, object_id=object_id)
            if not created:
